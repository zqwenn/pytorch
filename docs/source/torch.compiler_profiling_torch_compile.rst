...
torch.profiler is helpful for understanding the performance of your program at a kernel-level granularity - for example, it can show graph breaks and GPU, CPU and XPU utilization at the level of the program. The data provided by the profiler can often help users understand where to investigate further to understand model performance.

To understand kernel-level performance, other toosl exist. NVIDIA's ncu tool or Intel® VTune™ Profiler can be used, or :ref:`inductor's profiling tools <torchinductor-gpu-profiling>`.
